...
/**********************
*这个函数只作用于写请求，是写请求的位置分配函数，根据不同的分配策略将写请求映射到合适的物理位置上，
并在必要时产生读请求来处理写回操作
***********************/
Status allocate_location(struct ssd_info * ssd ,struct sub_request *sub_req)
{
	...
	if (ssd->parameter->allocation_scheme==0) /*动态分配的情况*/
	{
		...
		*所以需要产生一个读请求，并且只有这个读请求完成后才能进行这个页的写操作
		*******************************************************************/
		if (ssd->dram->map->map_entry[sub_req->lpn].state!=0)
		{
			//表示当前写请求(sub_req)中的数据状态与映射表中对应数据页的状态有差异，
			//即这些数据页需要被更新或修改。因此，需要产生读请求来读取旧数据，并根据写请求的更新数据来执行写回操作
		...
		****************************************/
		switch(ssd->parameter->dynamic_allocation)
		{
			...
			case 2:
			{
			sub_req->location->channel=ssd->token;   //记录着当前已经分配出去供程序使用的物理channel通道号
			ssd->token=(ssd->token+1)%ssd->parameter->channel_number;  //若使用则立即更新

			sub_req->location->chip=ssd->channel_head[sub_req->location->channel].token; //继续往下层级申请到plane
			ssd->channel_head[sub_req->location->channel].token=(sub_req->location->chip+1)%ssd->parameter->chip_channel[0];
	...
	{   /***************************************************************************
		*如果是静态分配方式的话，就可以将这个子请求的最终channel，chip，die，plane全部得出。
...
Status allocate_location_gc(struct ssd_info * ssd ,struct sub_request *sub_req)
{
	struct sub_request * update=NULL;
	unsigned int channel_num=0,chip_num=0,die_num=0,plane_num=0;
	struct local *location=NULL;
	...
	if ((ssd->parameter->allocation_scheme!=0)||(ssd->parameter->dynamic_allocation!=0))
	{
		if (ssd->channel_head[sub_req->location->channel].subs_w_head!=NULL)
		{
			sub_req->next_node = ssd->channel_head[sub_req->location->channel].subs_w_head;
            ssd->channel_head[sub_req->location->channel].subs_w_head = sub_req;
		}
		else
		{
...
Status  find_active_block_csb(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane)
{
	...
	for(count=0; count < ssd->parameter->block_plane; count++){
		...
		if(candidate_plane.blk_head[temp_block].fast_erase == TRUE) continue;
		//if(candidate_plane.blk_head[temp_block].free_lsb_num != 0) continue;
		if(candidate_plane.blk_head[temp_block].last_write_lsb < candidate_plane.blk_head[temp_block].last_write_csb) continue;     //为什么要找lsb更大的？？？
		if(candidate_plane.blk_head[temp_block].dr_state == DR_STATE_OUTPUT) continue;
...
/*=====================================================================================
*没有成功找到有空闲CSB page的块，接下来寻找有空闲MSB page的块
=====================================================================================*/
...
/**************************************************************************************
*函数的功能是寻找活跃块，因为每个plane中都只有一个活跃块，只有这个活跃块中才能进行操作
***************************************************************************************/
Status  find_active_block(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane)
{
	...
	while((free_page_num==0)&&(count<ssd->parameter->block_plane)&&(!gc_processing))//判断当前选中的块有空闲页且计数没有超过该plane的block数量上限且没有进行gc
	{
		//若没找到则往下一个块找
...
Status  find_active_block_dr(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane)
{
	...
	dr_state=ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[active_block].dr_state;      //跟上面那个函数相比只多了这一个条件：数据重组的状态
	//last_write_page=ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[active_block].free_page_num;
...
/*************************************************
*这个函数的功能就是一个模拟一个实实在在的写页面的操作
*具体操作结果就是更改这个page的相关参数，以及整个ssd的统计参数
**************************************************/
...
struct sub_request * creat_sub_request(struct ssd_info * ssd,unsigned int lpn,int size,unsigned int state,struct request * req,unsigned int operation, unsigned int target_page_type)
{
	...
	//若为空则说明创建的子请求是要挂到channel中的，程序会直接进行操作类型operation的判断。
	//把当前请求插入到req的子请求队列里
	if(req!=NULL)
	{
...
/*******************************************************************************
*函数的功能是寻找写子请求。并根据SSD的请求分配方式将其分配到相应的通道队列中。
根据不同的请求分配方式和更新页的状态，选择合适的写子请求，并将其取出放入相应通道的队列中，最终返回找到的写子请求的指针。
*分两种情况
...
*/
// 函数首先检查是否允许贪婪使用copyback高级命令（ssd->parameter->greed_CB_ad==1）。
// 如果允许，则根据之前的数据写入情况来决定是否使用copyback命令。
// 如果之前的逻辑页已经有写过的数据，且要写入的子请求中包含之前已经写过的数据，则可以直接使用copyback+random input命令完成写入，
// 否则需要先读取之前的数据，然后再执行copyback命令。

// 如果不允许贪婪使用copyback高级命令，则需要根据之前的数据写入情况来决定是否使用copyback命令。
// 如果之前的逻辑页已经有写过的数据，且要写入的子请求中包含之前已经写过的数据，则可以直接使用copyback命令完成写入，
// 否则需要先读取之前的数据，再执行普通写入操作。
...
/********************
写子请求的处理函数，分别处理了动态分配和静态分配策略下的写请求服务。
根据不同的策略和高级命令设置，选择合适的chip、plane和die，并处理相应的写子请求。
写请求的处理可能涉及到高级命令，如two_plane和interleave，也可能涉及到静态写和动态写的处理。
最终，如果有写请求被处理，则标记通道忙（channel_busy_flag = 1），否则保持通道空闲。函数返回SUCCESS表示写请求服务成功。
*********************/
Status  services_2_write(struct ssd_info * ssd,unsigned int channel,unsigned int * channel_busy_flag, unsigned int * change_current_time_flag)
{
	...
	*************************************************************************************************************************/
	...
	{
		if (ssd->parameter->allocation_scheme==1)                                       /*动态分配*/
		{
		...
		else if(ssd->parameter->allocation_scheme==0)                                     /*静态分配*/
		{
			for(chip=0;chip<ssd->channel_head[channel].chip;chip++)
			{
...
struct ssd_info *process(struct ssd_info *ssd)
{
	...
	*flag_die的值表示die号,old_ppn记录在copyback之前的物理页号，用于判断copyback是否遵守了奇偶地址的限制；
	...
	**********************************************************************************************************/
	...
	/*********************************************************
	*判断是否有读写子请求，如果有那么令flag为0，没有flag就为1
	*当flag为1时，即没有读写操作时若ssd中有gc操作这时就可以执行gc操作
	**********************************************************/
	for(i=0;i<ssd->parameter->channel_number;i++)  //判断所有channel中是否有读或写的子请求
	{
	...
	//上述情况操作结束(或者说是判断完，因为可能不满足条件没有进入具体执行路径)
	...
	for(chan=0;chan<ssd->parameter->channel_number;chan++)
	{
		...
		if((ssd->channel_head[i].current_state==CHANNEL_IDLE)||(ssd->channel_head[i].next_state==CHANNEL_IDLE&&ssd->channel_head[i].next_state_predict_time<=ssd->current_time))
		{
			...
			if (ssd->gc_request>0)
			{
				...
				if (flag_gc==1)/*执行过gc操作，需要跳出此次循环*/       //即此channel正忙，可以执行下一个通道的操作
				{
					continue;  //此时结束循环判断下一个channel
				}
			}
			//处于idle状态，但不满足GC要求，即没有进行GC操作
			...
			{
				services_2_r_data_trans(ssd,i,&flag,&chg_cur_time_flag);   //处理数据传输状态的读子请求
			}
			if(flag==0)                                                                  /*if there are no read request to take channel, we can serve write requests*/
			{
				//i表示通道channel；flag表示通道是否被占用(忙状态),0表示未被占用
				services_2_write(ssd,i,&flag,&chg_cur_time_flag);		//处理写子请求
			}
		}
	}
	return ssd;
...
Status get_ppn_for_normal_command(struct ssd_info * ssd, unsigned int channel,unsigned int chip, struct sub_request * sub)
{
	...
	if (ssd->parameter->allocation_scheme==DYNAMIC_ALLOCATION)       //如果是动态分配
	{
...
/***********************************************
*该函数的作用是让sub0，sub1的ppn所在的page位置相同
************************************************/
