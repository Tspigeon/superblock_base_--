...
*2.set the infomation according to the parameter file
    从参数文件中加载SSD的配置参数，包括通道数、芯片数、plane数、块数、页数等。
    初始化DRAM（动态随机存取存储器），DRAM是SSD中用于缓存数据的部分，初始化DRAM结构并为其分配内存空间。
    初始化SSD中的通道结构，为每个通道分配内存空间，并进行初始化设置。
    初始化SSD的其他成员变量，如时间相关的参数、请求计数等。
    打开输出文件，包括输出文件和统计文件，以便后续模拟运行过程中输出相关信息。
    读取参数文件的内容，并将其写入输出文件和统计文件中。
*******************************************/
struct ssd_info *initiation(struct ssd_info *ssd)
{
	...
	// gets(ssd->parameterfilename);
 	// strcpy(ssd->parameterfilename,16,"page.parameters");
    ...
    //初始化超级块
	ssd->superblock=(struct superblock_info*)malloc(ssd->parameter->chip_channel[0] * ssd->parameter->block_plane * sizeof(struct superblock_info));
	alloc_assert(ssd->superblock,"ssd->superblock");
	memset(ssd->superblock,0,ssd->parameter->chip_channel[0] * ssd->parameter->block_plane * sizeof(struct superblock_info));
	initialize_superblock(ssd);
	//====================================
	ssd->time_day = 0;
	ssd->current_time = 0;
    ...
    ssd->moved_page_time = 0;//计算的movegage迁移的时间
...
{
	unsigned int superblock_num = 0;
	superblock_num = ssd->parameter->block_plane * ssd->parameter->chip_channel[0];
    
	for(int i = 0;i < superblock_num;i++)
	{
		ssd->superblock[i].super_blk_loc = (struct superblock_channel*) malloc (ssd->parameter->channel_number * sizeof(struct superblock_channel));
		if(i< ssd->parameter->block_plane)
		{
            for (int j = 0; j < ssd->parameter->channel_number; ++j) {
			ssd->superblock[i].super_blk_loc[j].channel = j;
			ssd->superblock[i].super_blk_loc[j].blk = i;
		   }
		}
		else if(i >= ssd->parameter->block_plane)
		{
			for (int j = 0; j < ssd->parameter->channel_number; ++j) {
			ssd->superblock[i].super_blk_loc[j].channel = j;
			ssd->superblock[i].super_blk_loc[j].blk = i - ssd->parameter->block_plane;
		   }
	    }	
	}
	return ;
...
struct blk_info * initialize_block(struct blk_info * p_block,struct parameter_value *parameter)
{
    ...
    p_block->super_flag = 0;
	p_block->b_flag = 0;
	p_block->valid_page_num = 0;
